object Main {
	...
	def main(args: Array[String]) {
		...
		val sums = possibilities.groupBy(sum).view
		val products = possibilities.groupBy(product).view
		...
		val results = products2.values.flatMap(identity).toSet & sums2.values.flatMap(identity).toSet
